...
data <- soil
formula <- phi.formula <- phi.formula_null <- ~ DayAmdmt
formula_null <- ~ 1
link <- phi.link <- "logit"
...
corn_p <- parallel::parLapply(cl, seq(length(phyloseq::taxa_names(data))),
                              fun = doSim, data = data, formula = formula,
                              phi.formula = phi.formula, 
                              formula_null = formula_null,
                              phi.formula_null = phi.formula_null,
                              link = "logit", phi.link = "logit", B = 1000000)
parallel::stopCluster(cl)


save(corn_p, file =  "cornp.rda")
